...
<header>
		...
		<p align="center">Anda dapat memberikan Tugas kepada siswa dengan melengkapi form di bawah ini.</p>
			...
			if(isset($_POST["submit"])){
				...
				if($valid_matapel){				
				$insert = mysqli_query($koneksi,"INSERT INTO tugas (matapel,info,deadline,kelas,tgl,minat)
						   VALUES ('$matapel','$info','$deadline','$kelas','$tgl','$minat')") or die(mysqli_error());
				if($insert){
         ...
         <form class="form-horizontal" method="POST" enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
			...
			<label class="col-sm-5 control-label pad-right-zero">Matapel :</label>
			<div class="col-sm-5">
			...
			<div class="form-group form-group-sm">
			<label class="col-sm-5 control-label pad-right-zero">Kelas :</label>
			<div class="col-sm-5">
			...
			<div class="form-group form-group-sm">
			<label class="col-sm-5 control-label pad-right-zero">Info tugas :</label>
			<div class="col-sm-5">
				<textarea rows="3" cols="50" name="info" class="form-control input-sm" placeholder="INFO TUGAS"></textarea>
				</div>
			</div>
			<div class="form-group form-group-sm">
			<label class="col-sm-5 control-label pad-right-zero">Batas Pengumpulan :</label>
			<div class="col-sm-5">
				<fieldset>
